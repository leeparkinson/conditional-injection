...
while i<len(lines):
    ...
    para=[ln]; i+=1
    while i<len(lines) and lines[i].strip() and not re.match(r'^(#|\||\d+\. |- |\*Draft|\*\d{4}-|\*By )',lines[i]): para.append(lines[i]); i+=1
    text=' '.join(para)
...
date=re.search(r'^\*(?:Draft — )?(\d{4}-\d{2}-\d{2})\.',md,re.M).group(1)
bym=re.search(r'^\*By (.*)\*$',md,re.M); byline=('<p class="byline">'+inline(bym.group(1))+'</p>') if bym else ''
mast=f'<div class="page">\n<header class="mast"><p class="eyebrow">Preprint &middot; {date}</p><h1>{inline(title)}</h1><p class="eyebrow" style="margin:1.25rem 0 0">{inline(sub)}</p>{byline}</header>\n'
