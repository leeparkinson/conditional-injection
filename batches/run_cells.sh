#!/bin/bash
# Study 2 — clean replication of every reported cell (batches/cells.tsv), one paper section per batch
...
# Usage: batches/run_clean.sh [section-prefix]   e.g. batches/run_clean.sh 5.1   (default: all, in order)
set -u
...
sections=$(grep -v '^#' "$STUDY/batches/cells.tsv" | cut -f1 | awk '!s[$0]++')
for sec in $sections; do
  [ -n "$ONLY" ] && [[ "$sec" != "$ONLY"* ]] && continue
  lo=1001; case "$sec" in 8-baseline-b*) b=${sec##*b}; lo=$((1001+(b-1)*100));; esac
  jobs=$(grep -v '^#' "$STUDY/batches/cells.tsv" | awk -F'\t' -v s="$sec" '$1==s' | while IFS=$'\t' read -r _ probe tpl task models; do
  ...
  cp ~/.claude/.credentials.json "$ISO/.credentials.json"
  echo "$jobs" | xargs -P "$P" -n 5 "$STUDY/run_clean.sh"
